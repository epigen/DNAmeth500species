...
#Date: 26.07.2015
#Adaptet to calculate the diffMeth in Heart and Liver precisely on 18.12.2019
#options
...
output_dir=args[12]
  ...
  species="JS"
  wd=paste0("/scratch/lab_bock/shared/projects/compEpi/results_pipeline/", species)
  dedRef_id="toSelf_filtered_0.08mm_final_concat"
  sampleAnnotation=paste0("/scratch/lab_bock/shared/projects/compEpi/results_pipeline/", species, 
                           "/meta/sampleAnnotation.tsv")
  comp_col="new_comp"
  groups_col="Tissue"
  nTopDiffMeth=500
  scripts=file.path(Sys.getenv("CODEBASE"), "compEpi/src")
  motif="cpg"
  uc_tag="_uc"
  output_dir=paste0("/scratch/lab_bock/shared/projects/compEpi/results_analysis/03_motifAnalysis/diffMeth_additional_run/",
                   species)
...
#initialize
setwd(wd) ##to read, not to write!
theme_set(theme_bw())
options(stringsAsFactors=FALSE)

#system(paste0("mkdir -p ",getwd(),"/RCache/"))
dir.create(file.path(output_dir, "RCache"), recursive = T)
setCacheDir(file.path(output_dir, "RCache"))

#out_dir=paste0(RRBSdir,"/diffMeth_",motif)
#system(paste0("mkdir -p ",out_dir))
out_dir <- file.path(output_dir, RRBSdir)
dir.create(out_dir)
stats=data.frame()
...
collectMethData=function(dir,motif){
  RRBSfiles=system(paste0("ls ",dir, "/*/*/*",motif,"Methylation*.bed.gz"), intern=TRUE)
...
sample_annot$new_comp <- NA

##MODIFIED: now I need to compare only heart and liver (it is known that both are present in the sample)
sample_annot$new_comp[sample_annot$Tissue=="Heart"] <- "Heart"
sample_annot$new_comp[sample_annot$Tissue=="Liver"] <- "Liver"

if(!("Heart" %in% sample_annot$new_comp & "Liver" %in% sample_annot$new_comp)){
  print("either of Heart or Liver samples are not present. Exiting")
  quit()
}

#combine meth bed files by caching (Nathan)
simpleCache(recreate=FALSE,paste0(motif,"_combinedMeth_",species),instruction={collectMethData(file.path(wd,RRBSdir),motif)})
...
dir.create(file.path(output_dir, "motifAnalysis"))
if (length(na.omit(top1))>0){
  top_fa1=fasta[top1]
  writeXStringSet(top_fa1,paste0(output_dir, "/motifAnalysis/","ded_top",top_threshold,"_cov",cov_thres,"_",groups[1],".fa"))
  bottom_fa1=fasta[bottom1]
  writeXStringSet(bottom_fa1,paste0(output_dir,"/motifAnalysis/","ded_bottom",top_threshold,"_cov",cov_thres,"_",groups[1],".fa"))}else{message("No significantly hypermethylated fragments for ",comp[1])}
if (length(na.omit(top2))>0){
  top_fa2=fasta[top2]
  writeXStringSet(top_fa2,paste0(output_dir,"/motifAnalysis/","ded_top",top_threshold,"_cov",cov_thres,"_",groups[2],".fa"))
  bottom_fa2=fasta[bottom2]
  writeXStringSet(bottom_fa2,paste0(output_dir,"/motifAnalysis/","ded_bottom",top_threshold,"_cov",cov_thres,"_",groups[2],".fa"))}else{message("No significantly hypermethylated fragments for ",comp[2])}
...
writeXStringSet(fasta,paste0(output_dir,"/motifAnalysis/","allDeducedGenomeFragments.fa"))
