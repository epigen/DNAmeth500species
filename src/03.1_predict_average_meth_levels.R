...
wd = file.path(analysis_dir, "03_prediction_lm", "03.1_predictability")
dir.create(wd)
setwd(wd)
N_boot <- 10 #dev
...
summary_df<-fread(file.path(analysis_dir, "01_basicStats/01.10_all_seq_features/feature_summary_filtered.tsv"))
...
save(simple_lm, file = "model_simple.RData")
...
save(phylo_groups, file = "model_phylo_groups.RData")

#######tree
tree <- read.tree(file.path(analysis_dir, "01_basicStats", "01.6_ITOL","tree_species_as_tips_2021_2.phy"))
...
ggsave(paste0(target,"_phylfeatures_adjustedR2_", N_boot, ".pdf"), width = 6, height = 4)
...
ggsave(paste0(target,"_noprior_adjustedR2_", N_boot, ".pdf"), width = 6, height = 4)

## comparing AICs:
cat(paste0(c("prior", "first", "second", "rel.lik."), sep = "\t"), 
    file = "rel_likelihood.txt")
...
cat(paste0(c("\nno prior", "2mers", "3mers", e), sep = "\t"), 
    file = "rel_likelihood.txt", append = TRUE)
