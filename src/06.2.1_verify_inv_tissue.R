...
get_dataset<-function(ids, meth_data_mean, ded_ref, cgg_names, uc_map, isTest=FALSE, train_ids=NA){
  ...
  names <- c(c("meta","score","consN","mean_diffNts","max_diffNts","min_diffNts"),c(paste0(ids, ".cov"), 
                                                                                    paste0(ids, ".meth")))
  ...
  sub_df_cond <- sub_df_long[,list(Nsamples=.N,mean_cov=mean(cov),min_cov=min(cov),max_cov=max(cov),
...
train_models <- mclapply(seq_along(train_samples), function(i) 
  {simpleCache(cacheName=paste0("methTrain_",train_samples[[i]]),
               instruction = {train_test(train_data[[i]]$x, train_data[[i]]$y, type=train_sample_ids[[i]][[1]], 
                                ifRand=paste0("noRand", "Train"), run=0, k=kmer, subdir = subdir, 
                                SAVE_TRAIN_IDS = TRUE)},cacheDir=paste0(subdir,"/RCache"), assignToVariable="res",
               recreate=FALSE)})
...
in_other_sample<-function(SP){
  ...
  roc_dt_list=lapply(seq_along(rocdata_list), function(i) data.table(fdr=unlist(rocdata_list[[i]]@FPR),
                                                                     ...
                                                                     f1=f1_list[[i]], run=0, type=SP,
                                                                     ifRand="noRandTest", k=0, C=0,min_motif="NN",
                                                                     max_motif="NN", train_tissue=train_samples[[i]]))
  ...
  preddec_list_rand <- lapply(seq_along(train_models), 
                              function(i) predict(test_ds$x, object = train_models_rand[[i]]$model[[1]]))
  ...
  preddec_list_dec_rand <- lapply(seq_along(train_models_rand), 
                function(i) predict(test_ds$x, object=train_models_rand[[i]]$model[[1]], predictionType="decision"))
  rocdata_list_rand <- lapply(preddec_list_dec_rand, computeROCandAUC, y_rand, allLabels=c(-1,1))
  roc_dt_list_rand=lapply(seq_along(rocdata_list_rand), 
              function(i) data.table(fdr=unlist(rocdata_list_rand[[i]]@FPR),tpr=unlist(rocdata_list[[i]]@TPR),
                                     auc=unlist(rocdata_list_rand[[i]]@AUC),f1=f1_list[[i]], run=0, type=SP,
                                     ifRand="RandTest", k=0, C=0,min_motif="NN",max_motif="NN", 
                                     train_tissue=train_samples[[i]]))
...
res_sum <- res_full %>%
  dplyr::group_by(type, ifRand, train_tissue) %>% 
...
ggsave(file.path(subdir,  "AUC_in_other_sp.pdf"), plot = p, width = 6, 
       height = 2*length(unique(res_sum$train_tissue)), device = "pdf")
