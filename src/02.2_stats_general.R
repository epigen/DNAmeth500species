...
ggplot(stats_long_annot,aes(x=variable,y=value))+geom_violin()+geom_boxplot(outlier.shape=NA,fill="white",col="black")+xlab("")+facet_wrap(~variable,scales="free")
dev.off()
...
ggplot(sub_stats,aes(x=mapping_efficiency,y=others,col=species))+geom_point(alpha=0.6,size=1.2)+geom_abline(intercept=100,slope=-1)+ylab("% non-motif reads")+xlim(c(0,100))+ylim(c(0,100))
dev.off()
