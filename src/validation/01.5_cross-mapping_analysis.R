...
wd=file.path(analysis_dir, "validation", "01_crossMapping", "01.5_analysis",species, paste0(species, "_", genome_id) )
dir.create(wd, recursive = TRUE)
setwd(wd)


###reading resources in
genome_dir = file.path(data_dir, "resources", "reference_genomes", genome_id)
#files needed to find correct genome coordinates from concatenated genomes
chrom_sizes_file=file.path(genome_dir, paste0(genome_id, ".chrom.sizes"))
chrom_mapping_file=file.path(genome_dir, paste0(genome_id, "_chrom_mapping.tsv"))

#gene annotation file
ls_files = list.files(file.path(genome_dir, "tracks"))
bed_file = file.path(genome_dir, "tracks", ls_files[grepl("ref", ls_files, ignore.case=TRUE)])

#files from RefFreeDMA/cross-mapping
bam_file=file.path(processed_dir, species, "crossMapping", paste0(genome_id, "_concatinated"), "toSelf_filtered_0.08mm.bsmap.0.2.mism.1.r.0.n.bam")

meth_file=file.path(processed_dir, species, "toSelf_filtered_0.08mm_final_concat",
                    "diffMeth_cpg", paste0(species, "_mean_meth.tsv"))
...
pdf(paste0(species,"_methylation_profile.pdf"),width=9,height=4)
print(pl1)
print(pl1.1)
dev.off()
...
pdf(paste0(species,"_methylation_profile_merge.pdf"),width=9,height=4)
print(pl2)
print(pl2.1)
dev.off()
