...
treefile<-
tree <- read.tree("99.2_ITOL/species_tree_unranked_id.phy")
...
## if the ncbi_id is not a tip, that means that it ismapped internaly (and have the id INTXX)
...
split_tip<-function(df, tip_label, N_of_tips, sp_id){
  p<-df[df$label==tip_label, ]$parent
...
write.tree(tree_clean, paste0(analysis_dir,"/99.2_ITOL/tree_species_as_tips.phy"))


write.csv(df_new, paste0(analysis_dir,"/99.2_ITOL/tree_species_as_tips.csv"), row.names = F)


###saving the tree with the ordered species
d <- subset(fortify(tree_clean), isTip)
...
write.table(as.data.frame(sp_correct_order), file.path(Sys.getenv("CODEBASE"), 
                            "DNAmeth500species/meta/species_list_ordered.txt"), 
