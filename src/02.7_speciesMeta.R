...
wd=file.path(analysis_dir,"02_vizStats/02.7_speciesMeta")
...
stats_annot[color_class == "Jawless_vertebrate",group:="Inv."]
stats_annot[color_class == "Jawless_vertebrate",color_class:="Invertebrata"]
#plot stats for non-cpg methylation and pdr

#average per class
ggplot(stats_annot,aes(y=CpG_meth_cphpg,x=group,fill=color_class))+
  geom_boxplot(outlier.shape = 21)+
  scale_fill_manual(values = class_colors)+
  geom_point(data = stats_annot[species=="JL", ], aes(y=CpG_meth_cphpg,x=group), shape =21, fill = class_colors["Jawless_vertebrate"])+
  stat_summary(fun.data = give.n,fun.args = c(y=-0.1), geom = "text",size=2)+
  xlab("")+ylab("")

ggplot(stats_annot,aes(y=PDR_mean,x=group,fill=color_class))+
  geom_boxplot(outlier.shape = 21)+
 geom_point(data = stats_annot[species=="JL", ], aes(y=CpG_meth_cphpg,x=group), shape =21, fill = class_colors["Jawless_vertebrate"])+
  scale_fill_manual(values = class_colors)+
  stat_summary(fun.data = give.n,fun.args = c(y=-0.1), geom = "text",size=2)+
...
ggplot(stats_annot[Tissue%in%core_vert],aes(y=CpG_meth_cphph,x=Tissue,fill=color_class))+
  geom_boxplot(size=0.2,outlier.size = 0.5,outlier.shape = 21)+
  scale_fill_manual(values = class_colors)+
  stat_summary(fun.data = give.n,fun.args = c(y=-0.1), geom = "text",size=2)+
...
ggplot(stats_annot,aes(y=PDR_mean,x=CpG_meth,color=color_class))+
  geom_point(alpha=0.2)+
    geom_point(data = stats_annot[species=="JL", ], aes(y=PDR_mean,x=CpG_meth), color = class_colors["Jawless_vertebrate"], alpha = 0.2)+
  scale_color_manual(values = class_colors)+
...
pdf("MLTAW.pdf",7,6)
ggplot(rel_long,aes(x=MLTAW,y=value))+ geom_text(data=cor_c,vjust=1,hjust=0,aes(x=x,y=y,label=paste0('p=',signif(p_adj,3),'\nr=',signif(cor,3))),size=3)+
  geom_point()+geom_smooth(method = 'lm')+
  facet_wrap(variable~color_class, scale='free',nrow=3)
dev.off()


#association with cancer risk phyloglm
library(phylolm)

group="Aves"
#group="Mammalia"

tree=ape::read.tree(paste0(analysis_dir,"/01_basicStats/01.6_ITOL/tree_species_as_tips.phy"))

data=data.frame(rel_long[variable=="CpG_meth"&color_class==group&!duplicated(abbreviation_sp),c("abbreviation_sp","value","MLTAW"),],row.names = "abbreviation_sp")
colnames(data)[1]="CpG_meth"
head(data)

tree=drop.tip(tree,tip = tree$tip.label[!tree$tip.label%in%row.names(data)])
str(tree)

#check distribution. Here looks like gaussian
hist(data$MLTAW)

fit = compar.gee(MLTAW~CpG_meth,data=data,family="gaussian",phy=tree)
fit_nophy= glm(MLTAW~CpG_meth,data=data,family="gaussian")
