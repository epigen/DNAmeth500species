...
cov_min <- as.integer(args[[4]])
#kmer <- args[[3]]
#if (is.na(kmer)){kmer=c(1:2)}
kmer=c(1:10)
print(species)
print(cov_min)
...
get_train_test_split <- function(meth_data_mean_cond_red, ded_ref, N = 2000, SEED = "1234"){
  ...
  Nsel=min(c(min(table(meth_data_mean_cond_red[chr %in% chr_to_train, ]$category)), min(table(meth_data_mean_cond_red[chr %in% chr_to_test, ]$category))))
  if (Nsel > N/2) {
    Nsel = N/2
  }
...
##setup
path_to_folder <- paste0("../validation/WGBS_public/prediction/", species, "/", assembly ,"/")
...
if (!dir.exists(outdir)){
  dir.create(outdir)
  SEED = "1234" ## first time, reproducable seed
}else{
  SEED <- as.character(sample(1:1000, 1))
  outdir <- paste0(outdir, "_", SEED)
  print(outdir)
  dir.create(outdir)
}

##loading data
meth_data_mean = fread(file.path(path_to_folder, "mean_meth_per_fragment.tsv"))
...
##loading sequence
ded_ref=readDNAStringSet(file.path(path_to_folder, paste0(assembly, "_fragments.fa")))
...
#original labels
simpleCache(cacheName="methPred_noRand_uc", instruction={ train_test(x_train=split_ds$x_train,
...
rand_res_parallel = mclapply(rand_labs, function(labels_rand) {simpleCache(cacheName=paste0("methPred_Rand_uc_",
        labels_rand$name),
        instruction = {train_test(x_train = split_ds$x_train, x_test = split_ds$x_test,
         y_train=labels_rand$y_rand_train, y_test=labels_rand$y_rand_test, 
...
#saveRDS(rand_res_parallel, "model_rand.RDS")
...
auc_table <- fread("/nobackup/lab_bock/projects/DNAmeth500species/results_analysis_v1/02_predict_meth/02.1_within_species/summary/all_aucs.csv")
auc_res <- rbind(auc_res, list(ifRand="RRBS", auc = auc_table[species == abbreviation_sp, ]$AUC, x = 0.9, y = 0.17))
...
write.csv(roc_res, paste0(outdir,"/", assembly, "roc_res.csv"))
